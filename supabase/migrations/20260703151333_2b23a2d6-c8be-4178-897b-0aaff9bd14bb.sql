...
BEGIN
  ...
  UPDATE public.employees
     SET profile_id = p.id,
         email = p.email,
         phone = p.phone,
         position = COALESCE(p.position, public.employees.position),
         department = COALESCE(p.department, public.employees.department),
         birthday = COALESCE(p.birthday, public.employees.birthday),
         avatar = COALESCE(p.avatar_url, public.employees.avatar),
         organization_id = COALESCE(org_id, public.employees.organization_id),
         updated_at = now()
   WHERE profile_id IS NULL
     AND lower(name) = lower(p.name)
     AND (email IS NULL OR lower(email) = lower(p.email));
...
DROP TRIGGER IF EXISTS sync_employee_from_profile_on_profiles ON public.profiles;
CREATE TRIGGER sync_employee_from_profile_on_profiles
AFTER INSERT OR UPDATE OR DELETE ON public.profiles
FOR EACH ROW EXECUTE FUNCTION public.sync_employee_from_profile_trigger();

DROP TRIGGER IF EXISTS sync_employee_from_role_on_user_roles ON public.user_roles;
CREATE TRIGGER sync_employee_from_role_on_user_roles
